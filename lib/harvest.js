...
Harvest.harvestArea = async (lat, lng, radiusKm = 2) => {
  ...
  console.log('Harvest: inserting', segments.length, 'segments into database');
  const { error: insertErr } = await window.AppAPI.insertSegments(segments);
  if (insertErr) {
    console.error('Harvest: insert failed', {
      code: insertErr.code,
  ...
  console.log('Harvest: successfully inserted', segments.length, 'segments');
